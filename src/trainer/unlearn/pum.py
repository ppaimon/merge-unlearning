...
class PUM(FinetuneTrainer):
    ...
    def _apply_sigma_safety_clip(self) -> None:
        ...
        if sigma_rel_clip > 0.0 and base_rms > 0.0:
            ...
            if self.sigma_per_layer is not None:
                ...
                self.sigma = min(max(float(self.sigma or 0.0), 0.0), limit); clamped = True
            if clamped:
                logger.info("PUM: sigma clipped to <= %.6g (rel clip=%.3g, base_rms=%.6g)",
                            limit, sigma_rel_clip, base_rms)

        # min rel floor (new)
        if self.sigma_min_rel and self.sigma_min_rel > 0 and base_rms > 0.0:
            floor = self.sigma_min_rel * base_rms
            floored = False
            if self.sigma_per_layer is not None:
                new = []
                for s in self.sigma_per_layer:
                    ...
                    if s_val < floor:
                        s_val = floor; floored = True
                    new.append(s_val)
                self.sigma_per_layer = new
            else:
                s_val = float(self.sigma or 0.0)
                if s_val < floor:
                    self.sigma = floor; floored = True
            if floored:
                logger.info("PUM: sigma floored to >= %.6g (min rel=%.3g, base_rms=%.6g)",
                            floor, self.sigma_min_rel, base_rms)
            ...
            if round_norms_per_layer is not None:
                ...
                if self._layer_l2 is not None:
                    for li in range(len(Cs)):
                        L2 = float(self._layer_l2[li]) if li < len(self._layer_l2) else 0.0
    ...
    def _maybe_init_center_C_from_quantile(self) -> None:
        ...
        if all(len(v) == 0 for v in norms_per_layer) and self.center_clip_ref_synth_J > 0:
            if (self.synth_mode or "gaussian") == "gaussian":
                ...
                if self._layer_l2 is None:
                    logger.warning("PUM: layer L2 norms unavailable; gaussian synth fallback cannot scale per layer.")
                J = int(self.center_clip_ref_synth_J)
                for j in range(J):
                    for li in range(L):
                        L2 = float(self._layer_l2[li]) if (self._layer_l2 is not None and li < len(self._layer_l2)) else 0.0
                        norms_per_layer[li].append(rho * L2)
