...
#   • DeepSpeed-safe state dict handling + robust param-name canonicalization
...
def _near_identity_orthogonal(dim: int, eps: float, device, dtype):
    # 生成一个小角度正交矩阵：R = exp(eps * S)，S 是斜对称
    if eps <= 0:
        return torch.eye(dim, device=device, dtype=dtype)
    A = torch.randn(dim, dim, device="cpu", dtype=torch.float32)
    S = A - A.T  # 斜对称
    M = eps * S
    R = torch.linalg.matrix_exp(M)  # 近单位正交
    return R.to(device=device, dtype=dtype)

def _canon_name(n: str) -> str:
    # 统一去掉常见包装前缀/重复段
    #   - DeepSpeed/Accelerate: 'module.' 前缀
    #   - PEFT: 'base_model.' 前缀
    #   - 一些 wrapper 会产生 'model.model.' 的重复
    for pref in ("module.", "base_model."):
        if n.startswith(pref):
            n = n[len(pref):]
    n = n.replace("model.model.", "model.")
    return n

def _canonize_state_dict(sd: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    return {_canon_name(k): v for k, v in sd.items()}
...
def _state_dict_tensors(model: nn.Module) -> Dict[str, torch.Tensor]:
    # 注意：这里直接返回规范名
    return {_canon_name(n): p.detach().clone() for (n, p) in _all_named_params(model)}
...
def _apply_state_dict_delta(model: nn.Module, delta: Dict[str, torch.Tensor], scale: float = 1.0):
    with torch.no_grad():
        for n, p in _all_named_params(model):
            cn = _canon_name(n)
            if cn in delta:
                p.add_(scale * delta[cn].to(p.device))
...
def _generate_zero_sum_noises(
    ...
    any_pos = False
    for n, p in _all_named_params(model):
        if _std_for(_canon_name(n), p) > 0:
            any_pos = True
            break
    if not any_pos:
        return [_zero_like_param_dict(model, device=device) for _ in range(m)]

    param_names = [_canon_name(n) for (n, _) in _all_named_params(model)]
...
class PUM(FinetuneTrainer):
    """
    Perturb–Unlearn–Merge (PUM) meta
    ...
    def __init__(
        ...
        synth_disable_attn: bool = False,          # 是否在“合成参考”中禁用注意力正交
        synth_attn_eps: float = 0.03,              # 近单位正交的角度（越小越接近单位）
        synth_ffn_perm_frac: float = 0.25,         # 仅置换 FFN 中这部分通道（0~1）
        # base trainer args
        *args,
        **kwargs,
    ):
        ...
        method_args_override = method_args_override or {}
    ...
    # ----------------------------
    # DS/Accelerate helpers
    # ----------------------------
    @staticmethod
    def _accel_full_state_dict(model: nn.Module, accelerator) -> Dict[str, torch.Tensor]:
        """Return a consolidated (full) state dict with canonical names, even under ZeRO-3."""
        m = model
        if accelerator is not None:
            try:
                ...
                pass
            try:
                sd = accelerator.get_state_dict(m)
            except Exception:
                sd = m.state_dict()
        else:
            sd = m.state_dict()
    ...
    @staticmethod
    def _broadcast_model_to_all_ranks(model: nn.Module, src: int = 0) -> None:
        """Broadcast model parameters/buffers from src to all ranks (no-op if not distributed)."""
    ...
    def _make_inner_args(self, output_dir: str) -> TrainingArguments:
        ...
        if not (lr > 0):
            ...
            logger.warning(
                "PUM(inner): learning_rate was 0/None; defaulting to %.2e (set PUM_INNER_LR to override).",
                default_lr
            )
        # 调度器：短训避免 warmup 压成 0
    ...
    def train(self, resume_from_checkpoint: Optional[str] = None, **kwargs):
        ...
        # outer accelerator (if any)
        acc_outer = getattr(self, "accelerator", None)
        is_main = True
        try:
            is_main = bool(getattr(acc_outer, "is_main_process", True))
        except Exception:
            pass
        ...
        for r in range(1, self.rounds_R + 1):
            ...
            # Re-run DP calibration per round
            self._calibrate_sigma_from_dp()
            ...
            S0 = 0.0
            S1 = _zero_like_param_dict(self.model)           # 规范名键
            pub_sum_server = _zero_like_param_dict(self.model)
            cur_sd = _state_dict_tensors(self.model)         # 规范名键
            ...
            for k in range(self.copies_m):
                ...
                if self.jitter_rel_to_sigma > 0:
                    ...
                    for n, p in _all_named_params(self.model):
                        cn = _canon_name(n)
                        sig_n = per_param_sigma.get(cn, float(self.sigma or 0.0)) if per_param_sigma else float(self.sigma or 0.0)
                        tau_n = self.jitter_rel_to_sigma * sig_n
                        xi_k[cn] = _randn_like_with_generator(p, tau_n, rng) if tau_n > 0 else torch.zeros_like(p)

                # 先复制，再扰动
                model_k = copy.deepcopy(self.model)

                # 采样 T（训练中的重参开关受 use_orthogonal_reparam 控制；合成参考另说）
                ...
                # ---- BEFORE: get full (consolidated) state dict, aligned by T^-1
                acc = getattr(inner_trainer, "accelerator", None)
                before_sd_full = self._accel_full_state_dict(inner_trainer.model, acc)
                before_sd_aligned = self._apply_T_inv_to_update(before_sd_full, T_k)

                # 发布均值：只加在参数键上（与 pub_sum_server 对齐）
                with torch.no_grad():
                ...
                # 可选：PEFT 合并（当前默认 full-param，不触发）
                try:
                ...
                # ---- AFTER: get full state dict, align by T^-1, make delta
                after_sd_full = self._accel_full_state_dict(inner_trainer.model, acc)
                after_sd_aligned = self._apply_T_inv_to_update(after_sd_full, T_k)

                delta_k = _diff_state_dict(after_sd_aligned, before_sd_aligned)

                # 统计 overlap/nonoverlap（debug）
                overlap_norm_sq = 0.0
                ...
                if bad:
                    ...
                    del inner_trainer, model_k, after_sd_full, after_sd_aligned, delta_k
                    torch.cuda.empty_cache()
                    continue
            ...
            # 广播
            self._broadcast_model_to_all_ranks(self.model, src=0)
